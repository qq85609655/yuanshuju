 ...
 * 数据管理器，管理编辑器内所有的数据.
 */
Dep.framework.editor.base.Layer = Dep.framework.editor.base.EventSource
		.extend({
			...
			initStore : function(/* eventSource, model */) {
				var me = this, getAllProxy, getViewProxy, store, fTypes;
				fTypes = me.getModelManager().getTypesByGroup(
						me._getFGroupsArray());
				for ( var i in fTypes) {
			...
			_onStoreAdd : function(store, records, index, eOpts) {
				...
				me.fireEvent("modelCreated", me, store, records, index, eOpts);
			},
			/**
			 * 传递模型移除事件
			 * 
			...
			_onStoreRemove : function(store, record, index, isMove, eOpts) {
				...
				me.fireEvent("modelRemoved", me, store, record, index, isMove,
						eOpts);
			},
			// /**
			// * 大量模型移除事件
			...
			add : function(type, records) {
				...
				if (records instanceof Array) {
					for ( var i in records) {
						record = records[i];
						models = models.concat(me._add(store, record));
					}
				} else {
					models = models.concat(me._add(store, records));
				}
				return models;
			},
			 ...
			 */
			clearAll : function() {
				var me = this, store, keys = this.getStoreManager().keys(), key, length = keys.length;
				// for ( var key in storeManager) {
				for (var i = 0; i < length; i++) {
					// 获取store删除所有的数据
					key = keys[i];
					store = this.getDatasByType(key);
					if (store) {
						store.removeAll();
						store.setVisible(me.visible);
						store.setEditable(me.editable);
			...
			loadAll : function(view) {
				...
				$.when(deferreds).then(function(records) {
					...
					Dep.framework.editor.util.Msg.error('数据加载失败！', '错误');
				})/*
					 * .always(function() { me.logger.debug('不管成功失败，do it！'); })
					 */
			 ...
			 *            `true` ：仅加载界面数据, `false`加载业务数据和图形数据
			 */
			_createLoadDeferred : function(store, view) {
				var deferred;
				if (!store) {
					return;
				}
