...
Dep.framework.editor.plugin.containers.PropertiesWindow = Dep.framework.editor.plugin.ContainerPlugin
    .extend({
        ...
        _initView:function (pluginData) {
            ...
            me.setContainerPanel(propertyGrid);
...
/**
 * 属性改变命令。当属性值修改后，将此操作保存在命令集合中，用户可进行回退操作
 */
Dep.framework.editor.command.PropertiesChangeCommand = Dep.framework.editor.command.Command.extend({

    NAME:"Dep.framework.editor.command.PropertiesChangeCommand",
    
    init:function (grid,model, propertiesName, newValue, oldValue) {
        var me = this;
        me.grid = grid;
        me.model = model;
        me.propertiesName = propertiesName;
    ...
    execute:function () {
        var me = this;
        me.model.updateBussProperties(me.propertiesName, me.newValue);
        //以下代码删除，已经将此功能放到updateBussProperties接口中了
//        if (me.propertiesName == 'name') {//如果是name属性，更新name属性
//        	  me.model.set('name', me.newValue);
//        }
        //更新grid
        source = me.grid.getSource();
        source[me.propertiesName] = me.newValue;
    ...
    undo:function () {
        var me = this,source;
        me.model.updateBussProperties(me.propertiesName, me.oldValue);
        if (me.propertiesName == 'name') {//如果是name属性，更新name属性
      	  me.model.set('name', me.oldValue);
      }
        //更新grid
        source = me.grid.getSource();
        source[me.propertiesName] = me.oldValue;
        me.grid.setSource(source);
    },
